...
installMay mlib = do
  (shouldCreate, deps) <- (withUnknown $ D.depsAt depsFile) `catchError` askCreate
  ups <- execInstallM $ do
  ...
  where toInstall deps = case mlib of
          Nothing -> do
            liftIO $ putStrLn "Installing all declared dependencies..."
        ...
        askCreate err = do
          yes <- liftIO $ do
            putStrLn err
            putStr . unlines $ [ "Your project does not have a " ++ depsFile ++ " file yet, which is needed by the elm compiler to detect dependencies."
...
applyUpdates d up = (updateDeps . wrapAssoc) (unwrap up) d
  where updateDeps :: Functor f => ([(N.Name, V.Version)] -> f [(N.Name, V.Version)]) -> D.Deps -> f D.Deps
        ...
        unwrap (Update m) d = do
          (Endo f) <- m
          return $ f d

install1 :: Bool -> D.Deps -> Library -> InstallM ()
install1 shouldAsk oldDeps l@(Lib.Library name _) = do
  finalVsn <- Cmd.inDir EPath.dependencyDirectory $ do
    (repo,version) <- lift $ Cmd.inDir Path.internals $ getRepo l
...
getRepo l = do
  let directory = N.toFilePath . Lib.lib $ l
...
getVersion (Lib.Library name mayVsn) = do
...
-- updateDepsFile :: InstallM ()
-- updateDepsFile isNewLib = do
--   (shouldAsk, deps, l) <- ask
--   let nm      = name l
--       newVsn  = version l
--       depsMap = D.dependencies deps
--       moldVsn = lookup nm depsMap
--   (yes, resp) <- case moldVsn of
--     Nothing -> shouldAdd
--     Just oldVsn  ->
--       unless (oldVsn == newVsn) (shouldUpdate oldVsn newVsn l)
--   if yes
--     then (tell $ updateDeps l)
--     else putStr resp
     
--   where
--     shouldAdd = do
--       yes <- shouldI $ 
--       return (yes, didntUpdateMsg)
--     shouldUpdate oldVsn newVsn = do
--       yes <- shouldI $ 
--       return (yes, )
--     depsFile = EPath.dependencyFile
